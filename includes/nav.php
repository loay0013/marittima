
<nav class="navbar navbar-expand-xl navbar-dark bg-transparent position-absolute w-100 z-index pt-0">
    <div class="container-fluid d-flex  align-items-center p-0">
        <a class="text-decoration-none" href="index.php">
            <div class="d-xl-none p-2">
                <p class="text-light logo-text align-items-center m-0">
                    MARITTIMA
                </p>
            </div>

        </a>
        <button class="navbar-toggler border-0 shadow-none "  type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse justify-content-center" id="navbarNav">
            <ul class="nav flex-column flex-xl-row tlf m-0">
                <li class="nav-item pb-2 p-xl-2 ">
                    <a class="nav-link links" aria-current="page" href="index.php">FORSIDE</a>
                </li>
                <li class="nav-item pb-2 p-xl-2">
                    <a class="nav-link links" href="Vinoteque.php">VINOTEQUE</a>
                </li>
                <li class="nav-item pb-2 p-xl-2">
                    <a class="nav-link links" href="Booking.php">BOOKING</a>
                </li>
                <li class="nav-item pb-2 p-xl-2">
                    <a class="nav-link links" href="Private%20%20events.php">PRIVATE EVENTS</a>
                </li>

                <li class="nav justify-content-center p-xl-3">
                    <a class="justify-content-center d-flex" href="index.php">
                        <div class="d-xl-block d-none">
                            <img src="images/logo-nav-D.png" alt="logo til navbar">

                        </div>

                </li>

                <li class="nav-item pb-2 p-xl-2">
                    <a class="nav-link links" href="Events.php">EVENTS</a>
                </li>
                <li class="nav-item pb-2 p-xl-2">
                    <a class="nav-link links" href="Historien.php">HISTORIEN</a>
                </li>
                <li class="nav-item pb-2 p-xl-2">
                    <a class="nav-link links" href="Maps.php">FIND OS</a>
                </li>
                <li class="nav-item pb-2 p-xl-2">
                    <a class="nav-link links" href="Kontakt.php">KONTAKT OS</a>
                </li>
            </ul>
        </div>
    </div>
</nav>


